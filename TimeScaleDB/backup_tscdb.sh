#### backup timescale db  backup_tscdb.sh ###
...
backup_file="${local_backup_path}/all-databases-$(date +%Y-%m-%d_%Hh%Mm).sql.gz"
keep_day=3
backup_file_size=0
file_size_min=1000
...
# Create a backup
if PGPASSWORD="${backup_pass}" pg_dumpall -U "${backup_user}" -h "${server_ip}" | gzip > "${backup_file}"; then
  backup_file_size=$(wc -c "${backup_file}" | awk '{ print $1}')
  if [[ ${backup_file_size} -lt ${file_size_min} ]]; then
...
if [ "$env" == "Production" ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    backup_error_data=$(generate_post_data "No ${db_type} backup file on servers ${server_ip} was syndicated!" "14177041" "${backup_file_size}" "${end_time}")
    curl -H "Content-Type: application/json" -X POST -d "${backup_error_data}" "${discord_error_webhook}"
    die "No ${db_type} backup file on servers ${server_ip} was syndicated! \n"
  fi
